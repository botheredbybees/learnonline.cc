...
from models.schemas import (
    UnitSchema,
    UnitElementSchema,
    UnitPerformanceCriteriaSchema,
    UnitCriticalAspectSchema,
    UnitRequiredSkillSchema,
)
import os
...
async def get_available_units(
    ...
    if not hasattr(current_user, "role") or current_user.role.name != "admin":
        raise HTTPException(
            status_code=403, detail="Only admin users can access available units"
        )
    ...
    if not username or not password:
        raise HTTPException(
            status_code=500, detail="TGA API credentials not configured"
        )
    ...
    try:
        ...
        for unit in tga_units:
            existing_unit = (
                db.query(models.Unit).filter(models.Unit.code == unit["code"]).first()
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve available units: {str(e)}"
        )
...
async def bulk_download_units(
    ...
    if not hasattr(current_user, "role") or current_user.role.name != "admin":
        raise HTTPException(
            status_code=403, detail="Only admin users can perform bulk downloads"
        )

    if not unit_codes:
        raise HTTPException(status_code=400, detail="No unit codes provided")
    ...
    background_tasks.add_task(
        download_manager.process_units_download, job_id, unit_codes, current_user.id
    )
...
async def get_units_download_status(
    job_id: str, current_user: models.User = Depends(get_current_user)
):
    """Get the status of a units bulk download job (admin only)"""
    # Check if user has admin permissions
    if not hasattr(current_user, "role") or current_user.role.name != "admin":
        raise HTTPException(
            status_code=403, detail="Only admin users can check download status"
        )
...
async def get_unit_elements(unit_id: int, db: Session = Depends(get_db)):
    ...
    return (
        db.query(models.UnitElement).filter(models.UnitElement.unit_id == unit_id).all()
    )


@router.get(
    "/{unit_id}/performance-criteria",
    response_model=List[UnitPerformanceCriteriaSchema],
)
async def get_unit_performance_criteria(unit_id: int, db: Session = Depends(get_db)):
    ...
    return (
        ...
        .filter(models.UnitPerformanceCriteria.unit_id == unit_id)
        .all()
    )


@router.get(
    "/{unit_id}/critical-aspects", response_model=List[UnitCriticalAspectSchema]
)
async def get_unit_critical_aspects(unit_id: int, db: Session = Depends(get_db)):
    ...
    return (
        db.query(models.UnitCriticalAspect)
        .filter(models.UnitCriticalAspect.unit_id == unit_id)
        .all()
    )
...
async def get_unit_required_skills(unit_id: int, db: Session = Depends(get_db)):
    ...
    return (
        db.query(models.UnitRequiredSkill)
        .filter(models.UnitRequiredSkill.unit_id == unit_id)
        .all()
    )


@router.get("/{unit_id}/elements-with-pc")
async def get_unit_elements_with_performance_criteria(
    ...
    elements = (
        ...
        .filter(models.UnitElement.unit_id == unit_id)
    ...
    for element in elements:
        ...
        pcs = (
            db.query(models.UnitPerformanceCriteria)
            .filter(models.UnitPerformanceCriteria.element_id == element.id)
            .order_by(models.UnitPerformanceCriteria.pc_num)
            .all()
        )

        element_dict["performance_criteria"] = [
            {"id": pc.id, "pc_num": pc.pc_num, "pc_text": pc.pc_text} for pc in pcs
        ]
...
async def get_unit_comprehensive(unit_id: int, db: Session = Depends(get_db)):
        ...
        elements_data.append(
            {
                ...
                "element_text": element.element_text,
                "performance_criteria": [
                    {"id": pc.id, "pc_num": pc.pc_num, "pc_text": pc.pc_text}
    ...
    critical_aspects = (
        db.query(models.UnitCriticalAspects)
        .filter(models.UnitCriticalAspects.unit_id == unit_id)
        .all()
        if hasattr(models, "UnitCriticalAspects")
        else []
    )

    # Get required skills (when implemented)
    required_skills = (
        db.query(models.UnitRequiredSkills)
        .filter(models.UnitRequiredSkills.unit_id == unit_id)
        .all()
        if hasattr(models, "UnitRequiredSkills")
        else []
    )
    ...
    return {
        ...
        "critical_aspects": [
            {"id": ca.id, "aspect_text": ca.aspect_text} for ca in critical_aspects
        ],
        "required_skills": [
            {"id": rs.id, "skill_text": rs.skill_text} for rs in required_skills
        ],
    }
...
async def search_units(
    ...
    # Search local database using ORM filter instead of raw SQL
    ...
    return local_units_dict + [
        u
        for u in tga_units
        if not any(l["code"] == u["code"] for l in local_units_dict)
    ]
...
async def sync_unit(
    ...
    # Check if user has admin permissions
    if not hasattr(current_user, "role") or current_user.role.name != "admin":
        raise HTTPException(status_code=403, detail="Only admin users can sync units")
    ...
    if not unit_data or not isinstance(unit_data, dict):
        raise HTTPException(
            status_code=404, detail=f"Unit {unit_code} not found in TGA"
        )
    ...
    def process_elements(unit_id, unit_code: str):
        ...
        try:
            ...
            if elements_data and isinstance(elements_data, dict):
                ...
                unit = (
                    session.query(models.Unit).filter(models.Unit.id == unit_id).first()
                )
                if unit:
    ...
    if unit:
        try:
            ...
            unit_db = (
                db.query(models.Unit).filter(models.Unit.code == unit_code).first()
            )
            if unit_db:
                unit_id = unit_db.id

    return {"message": f"Unit {unit_code} sync started", "unit_id": unit_id}
...
async def set_unit_visibility(
    ...
    if not hasattr(current_user, "role") or current_user.role.name != "admin":
        raise HTTPException(
            status_code=403, detail="Only admin users can modify unit visibility"
        )
