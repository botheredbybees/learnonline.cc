...
def send_email(to_email: str, subject: str, body: str):
    ...
    try:
        if (
            not EMAIL_AVAILABLE or not SMTP_USERNAME
        ):  # Skip email sending in development/testing
...
def create_user_profile(db: Session, user_id: UUID) -> UserProfile:
    ...
    profile = UserProfile(user_id=user_id, experience_points=0, level=1)
    db.add(profile)
    db.commit()
    db.refresh(profile)
    return profile
...
async def reset_password(
    reset_data: PasswordResetSchema, db: Session = Depends(get_db)
):
    """
