...
psql -U ${DB_USER} -d ${DB_NAME} -h ${DB_HOST:-localhost} -p ${DB_PORT:-5432} -f schema_updates/update_user_levels.sql

echo "Creating user activity logs table..."
psql -U ${DB_USER} -d ${DB_NAME} -h ${DB_HOST:-localhost} -p ${DB_PORT:-5432} -f schema_updates/create_activity_logs.sql
